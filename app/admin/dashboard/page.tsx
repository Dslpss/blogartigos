...
import { getBlogSettings, updateBlogSettings, getArticles, BlogPost, addArticle } from '@/lib/db';
                  ...
                  {isAddingArticle ? (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                         ...
                         <textarea required value={newArticle.content} onChange={e => setNewArticle({...newArticle, content: e.target.value})} className="w-full bg-white border border-border rounded-xl p-3 outline-none h-40" />
                      </div>
                      <div className="flex items-center gap-4 pt-4">
                        <button type="submit" disabled={saving} className="btn-premium py-3 px-10">
                          {saving ? 'Publicando...' : 'Publicar Artigo'}
                        </button>
                        <button type="button" onClick={() => setIsAddingArticle(false)} className="bg-border text-primary font-bold px-6 py-3 rounded-xl uppercase text-[10px] tracking-widest">
                          Cancelar
                        </button>
                      </div>
                    </motion.form>
